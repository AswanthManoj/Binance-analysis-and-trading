...
def analyse(symbol, timeframe):
    ...
    try:
        ...
        if timeframe==config.timeFrame_main:            
            ...
            dm['sig'] = dm.macd.ewm(span = 9, adjust = False).mean()#dm['hist'] = dm['macd'] - dm['sig']
            #atr
            ...
            #trailing stoploss
            if (inPosition['inTrade']==True) and (timeframe==config.timeFrame):
                trailing = max(asset_data['trailing'],dm['stoploss'].iloc[-1])
            else:
                trailing=0
            #rsi
            #rsi = RSIIndicator(dm['close'])
            #dm['rsi'] = rsi.rsi()
            #dm['rsi_avg'] = dm['rsi'].rolling(config.rsi_avg_len).mean()
            if (dm['hist'].iloc[-3]>dm['hist'].iloc[-2]) and (dm['hist'].iloc[-2]<dm['hist'].iloc[-1]):
                if (dm['hist'].iloc[-3]<0) and (dm['hist'].iloc[-2]<0):
                    signal_data = True
                #elif (dm['hist'].iloc[-3]>0) and (dm['hist'].iloc[-2]>0) and (dm['hist'].iloc[-1]>0) and :
                #    signal_data = True
    ...
    try:
        ...
        if config.isTest:
            ...
            if (inPosition['inTrade']==False):
                ...
                if (is_candle_closed==True) and (tframe==config.timeFrame):# and (orderData['stoploss']<(orderData['price']*1.0025)): 
