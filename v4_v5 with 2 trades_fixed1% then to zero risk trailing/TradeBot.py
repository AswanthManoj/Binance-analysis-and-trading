...
def sendNotification(_message: str):
    try:
        pass#push = pb.push_note(title, _message)
    except:
        pass

def supertrend(df, period=100, atr_multiplier=3):
    try:
...
def analyse(symbol, timeframe):
    global signal_data, asset_data, inPosition, trend_signal_data, orderData
    try:
        _filename = '_market_data_csv/'+symbol+'_'+timeframe
        d = pd.read_parquet(_filename+'.gzip')
        dm = pd.DataFrame()
        ...
        if timeframe==config.timeFrame_main:            
            ...
            if dm['in_uptrend'].iloc[-1]<dm['close'].iloc[-1]:
                trend_signal_data = True
            else:
                trend_signal_data = False
        else:
            #macd
            ...
            dm['signal_line'] = ((dm['atr_avg']+dm['hist'])-dm['atrn'])/3
            ...
            dm['stoploss_2'] = dm['close']-(config.atrStoploss_2*dm['atr'])
            dm['takeprofit'] = dm['close']+(config.atrTakeprofit*dm['atr'])
            #trailing stoploss
            if (inPosition['inTrade']==True) and (inPosition['tradeSwitch']==1) and (timeframe==config.timeFrame):
                trailing = max(asset_data['trailing'],dm['stoploss'].iloc[-1])
            elif (inPosition['inTrade']==True) and (inPosition['tradeSwitch']==2) and (timeframe==config.timeFrame):
                trailing = max(asset_data['trailing'],dm['stoploss_2'].iloc[-1])
            else:
                trailing=0
            if (dm['hist'].iloc[-3]>dm['hist'].iloc[-2]) and (dm['hist'].iloc[-2]<dm['hist'].iloc[-1]):
                if (dm['hist'].iloc[-3]<0) and (dm['hist'].iloc[-2]<0):
                    signal_data = True
                elif (dm['hist'].iloc[-3]>0) and (dm['hist'].iloc[-2]>0) and (dm['hist'].iloc[-1]>0) and (dm['hist'].iloc[-1]<dm['signal_line'].iloc[-1]): # secondary help signaal
                    signal_data = True
...
def _test_checkBuy(symbol, price: float):
    ...
    try:
        ...
        stlos_atr = float(truncateToPrecision( str(asset_data['stoploss']), price_precisions ))
        #tkpro_atr=round(asset_data['takeprofit'],price_precisions)
        tkpro_atr = float(truncateToPrecision( str(asset_data['takeprofit']), price_precisions ))
...
def _test_takeprofit(symbol, price: float):
    global orderData, inPosition, asset_data, usdt_test_profits, signal_data, trend_signal_data
    try:
        if (inPosition['tradeSwitch']==1):
            usdt_test_profits = _placeTestOrder(symbol, price, 'sell', (orderData['assetQuantity']/2))
            inPosition['tradeSwitch'] = 2
            orderData['assetQuantity'] = (orderData['assetQuantity']/2)
            orderData['takeprofit'] = asset_data['takeprofit']*1.03
            asset_data['trailing'] = orderData['price']
        elif (inPosition['tradeSwitch']==2):
            usdt_test_profits = _placeTestOrder(symbol, price, 'sell', (orderData['assetQuantity']))+usdt_test_profits
...
def on_message(ws, message):
    global timeFrameData, orderData, inPosition, asset_data, signal_data, title, price_precisions, usdt_test_profits, prevBal, trend_signal_data
    try:
        ...
        if config.isTest:
            ...
            if (is_candle_closed==True) and (tframe==config.timeFrame):
                update(candle,smb,config.timeFrame)
                asset_data['stoploss'], asset_data['takeprofit'], asset_data['trailing'] = analyse(smb,config.timeFrame)
                price_precisions = max( price_precisions, _round_precision )
            elif (is_candle_closed==True) and (tframe==config.timeFrame_main):
                update(candle,smb,config.timeFrame_main)
                analyse(smb,config.timeFrame_main) 

            if (inPosition['inTrade']==False):
                if (trend_signal_data) and (inPosition['tradeSwitch']==0) and (signal_data):
                    if( config.AcBalance>(1.1*config.tradeCapital) ): 
                        _test_checkBuy(smb,price)
                        log(smb+'\n'+str(orderData))
                        print(smb+' bought', orderData)
                ...
                if (is_candle_closed==True) and (tframe==config.timeFrame): 
                    sl = float(truncateToPrecision( str(asset_data['trailing']), price_precisions ))
                    orderData['stoploss'] = max(sl,orderData['stoploss'])
                if (inPosition['tradeSwitch']==1):  
                    if( price>=orderData['takeprofit'] ):
                        profit = _test_takeprofit(smb,price) 
                        print('Partial Profit taken', profit, config.tradeData, config.AcBalance) 
                        log('Partial profit taken\n'+'Profit : '+str(profit)+', Account Balance : '+str(config.AcBalance)+str(config.tradeData)+'\n')
                        _testtrackPerformance(time,price)
                        sendNotification(_test_message(orderData,'partial profit'))
                    elif( price <= orderData['stoploss'] ) and is_candle_closed:
                        profit = _test_stoploss(smb,price) 
                    ...
                    if( price <= orderData['stoploss'] ) and is_candle_closed:
                        profit = _test_stoploss(smb,price) 
                        print('Sold', profit, config.tradeData, config.AcBalance) 
                        log('Sold\n'+'Profit/Gain : '+str(profit)+', Account Balance : '+str(config.AcBalance)+str(config.tradeData)+'\n')
                        _testtrackPerformance(time,price)
                        sendNotification(_test_message(orderData,'sold', profit))
...
#global variables
global base, quote, orderData, inPosition, asset_data, signal_data, title, price_precisions, usdt_test_profits, prevBal, trend_signal_data
base=config.symbol.upper()[:-4]
quote=config.symbol.upper()[-4:]
title = 'SCALPER_BOT_'+base+'-'+quote+'-updated'
asset_data = {                                                                                                                                                   
    'stoploss'      : 0.0,
    'takeprofit'    : 0.0,
    'trailing'      : 0.0                                                                                                                              
...
trend_signal_data=False
price_precisions=0
inPosition={'inTrade':False,'tradeSwitch':0}
orderData = {'position':False, 'assetQuantity':0.0, 'price':0.0, 'stoploss':0.0, 'takeprofit':0.0} 
usdt_test_profits=0                                                                                                                                                                                                                                                                                                                   
try:
    ...
    while(True):                                                                                                                                                                
        try:                                                                                                                                                                       
            if( config.AcBalance>(1.1*config.tradeCapital) ):                                                                                                                                                                                             
                ...
                _createtrackfile()
                symbol_prev = config.symbol.upper()
                symbol_prev=symbol_prev[:-4]+'/'+symbol_prev[-4:]
                p,t,price_precisions=loadData(config.symbol,config.timeFrame,symbol=symbol_prev)
                loadData(config.symbol,config.timeFrame_main,symbol=symbol_prev)
                url=url+config.symbol+"@kline_"+config.timeFrame+"/"+config.symbol+"@kline_"+config.timeFrame_main+"/"
                config.assetForTracking=config.AcBalance/p
                _testtrackPerformance(t,p)  
                asset_data['stoploss'], asset_data['takeprofit'], asset_data['trailing'] = analyse(config.symbol,config.timeFrame)
                analyse(config.symbol,config.timeFrame_main)
